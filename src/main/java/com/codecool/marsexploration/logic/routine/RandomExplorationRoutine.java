...
public class RandomExplorationRoutine implements Routine {
    ...
    public void move(Context context) {
        ...
        if (context.getStepNumber() == 0) {
            context.getRover().addExploredCoordinates(context.getLandingCoordinate());
            context.getRover().setPosition(context.getLandingCoordinate());
        } else {
            context.getRover().addExploredCoordinates(newRoverPosition);
            context.getRover().setPosition(newRoverPosition);
        }
    }
}
